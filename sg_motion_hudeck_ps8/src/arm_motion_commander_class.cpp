...
int ArmMotionCommander::plan_move_to_pre_pose(void) {
    ...
    ROS_INFO("finished before timeout");
    if (cart_result_.return_code==7){//cwru_action::cwru_baxter_cart_moveResult::RT_ARM_PATH_NOT_VALID) {
        ROS_WARN("right arm plan not valid");
        return (int) cart_result_.return_code;
    }
    if (cart_result_.return_code!=9){//cwru_action::cwru_baxter_cart_moveResult::SUCCESS) {
