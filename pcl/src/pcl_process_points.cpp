...
void searchThroughPoints(){

	//fit the points to plane via pcl_util function
    cwru_pcl_utils.fit_points_to_plane(g_pclKinect, &plane_normal, &plane_dist);
    
    //now we have transformed the cloud
    cwru_pcl_utils.transform_cloud(A, g_pclKinect,  cloud_transformed); 

    for(int i = 0; i < cloud_transformed->points.size(); ++i){
    	if(cloud_transformed.points[i]. /*how do I get the z value */ =  /* reference z value ( optional +- some difference ) */  ){
